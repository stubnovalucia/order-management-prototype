    ...
    <div class="relative p-4 w-full max-w-3xl h-full md:h-auto">
      ...
      <div class="relative p-4 bg-white rounded-lg shadow sm:p-5">
        ...
        <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5">
          <h3 class="text-lg font-semibold text-gray-900">Nová objednávka</h3>
          <button
            type="button"
        ...
        <form @submit.prevent="handleSubmit">
          <div class="grid gap-4 mb-4 sm:grid-cols-2">
            <div>
              ...
              <span v-if="!newOrder.category" class="text-red-500 text-sm"
                >Kategória je povinná</span
              >
            </div>
            <div>
              ...
              <span v-if="!newOrder.brand" class="text-red-500 text-sm">Značka je povinná</span>
            </div>
            <div>
              ...
              <span v-if="!newOrder.status" class="text-red-500 text-sm">Stav je povinný</span>
            </div>
            <div>
...
const priceError = computed(() => newOrder.value.price <= 0)

const isFormValid = computed(() => {
  return (
    newOrder.value.category && newOrder.value.brand && newOrder.value.status && !priceError.value
  )
})

const emits = defineEmits<{
  (e: 'close'): void
  (e: 'createOrder', order: typeof newOrder.value): void
}>()
...
const handleSubmit = () => {
  ...
  emits('createOrder', newOrder.value)
  newOrder.value = {
    category: '',
    brand: '',
    status: '',
